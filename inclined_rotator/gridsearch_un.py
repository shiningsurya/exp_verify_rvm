...
with np.load ("restream.npz") as rs:
    ...
    LSIZE    = LONGS.size
    DATA_III = np.array ( rs['fI'][SS] )
    DATA_IPQ = np.array ( rs['I+Q'][SS] )
    DATA_IMQ = np.array ( rs['I-Q'][SS] )
    DATA_IPU = np.array ( rs['I+U'][SS] )
    DATA_IMU = np.array ( rs['I-U'][SS] )
################################
### create grid
def get_grids ( asize=512, psize=512 ):
    AMPG   = np.linspace ( 0., 1, asize )
